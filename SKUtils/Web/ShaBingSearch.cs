...
{
    public ShaBingSearch(ShaBingSearchConfig? config = null)
    {
        this._logger =
            config?.LoggerFactory?.CreateLogger(typeof(ShaBingSearch)) ?? NullLogger.Instance;
        this._shaBingSearchCore = new ShaBingSearchCore(
            config?.Host?.ToString(),
            config?.Timeout,
            _logger
        );
        this._stringMapper = config?.StringMapper ?? s_defaultStringMapper;
        this._resultMapper = config?.ResultMapper ?? s_defaultResultMapper;
    }
