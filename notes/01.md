...
- **利用 Semantic Kernel：**步骤可以利用一个或多个内核函数，使您能够在流程中利用语义内核的所有方面。
- **可重用性和灵活性：**步骤和流程可以在不同的应用程序中重复使用，从而促进模块化和可扩展性。
- **事件驱动型架构：**利用事件和元数据有效地触发操作和流程步骤之间的转换。
- **完全控制和可审计性：**以定义和可重复的方式保持对流程的控制，并通过 Open Telemetry 完成审计功能。

**使用场景**

- **开户：**此过程包括多个步骤，例如信用提取和评级、欺诈检测、在核心系统中创建客户账户以及向客户发送欢迎信息，包括他们的客户 ID。
- **送餐：**订购外卖是一个熟悉的过程。从通过电话、网站或应用程序接收订单，到准备每道食品，确保质量控制、司机分配和最终交付，这个过程中的许多步骤都可以简化。
- **支持票证：**我们都提交了支持票证 — 无论是针对新服务、IT 支持还是其他需求。此流程可能涉及基于业务和客户需求的多个子流程，最终通过有效满足客户需求来实现满意度。
...
flowchart LR
    ...
    End["End"]
    IntroStep["IntroStep"]
    IntroStep["IntroStep"] --> ChatUserInputStep["ChatUserInputStep"]
    ChatUserInputStep["ChatUserInputStep"]
    ChatUserInputStep["ChatUserInputStep"] --> End["End"]
    ChatUserInputStep["ChatUserInputStep"] --> ChatBotResponseStep["ChatBotResponseStep"]
    ChatBotResponseStep["ChatBotResponseStep"]
    ChatBotResponseStep["ChatBotResponseStep"] --> ChatUserInputStep["ChatUserInputStep"]
    Start --> IntroStep["IntroStep"]
    ChatBotResponseStep["ChatBotResponseStep"] --> End
```
...
- **过程（Process）**：为实现特定目标而设计的一系列步骤。这些步骤以这样一种方式相互连接，以便它们可以通过发送和接收事件进行通信。步骤之间的连接是在流程创建期间建立的。
- **步骤（Steps）**：流程中的各个活动，每个活动都有定义的输入和输出，有助于实现总体目标。现有流程可用作另一个流程中的步骤。步骤主要有两种类型：
  - **无状态步骤（Stateless Steps）**：这些步骤不会在执行之间保留任何信息。它们独立运行，无需存储状态数据。
  - **有状态步骤（Stateful Steps）**：这些步骤维护可持久的状态，从而允许在流程的后续运行中重复使用和更新状态。这些步骤的状态可以存储和序列化。

- 通常，流程和步骤都设计为可在不同的流程中重复使用。
...
账户开设示例包含了两种不同的实现方式，覆盖相同的场景，只是使用了不同的SK组件来达到同样的目标。

此外，该示例介绍了使用较小的进程作为步骤的概念，以保持主进程的可读性和可管理性，便于未来的改进和单元测试。同时也介绍了SK事件订阅者的使用。
...
一个仅简单连接上述步骤（未使用子进程作为步骤）的SK流程用于开设账户看起来像这样：[此处应跟随具体的流程描述或图表，但根据提供的信息并未给出具体流程细节]。

这种翻译保留了原文的技术术语，确保技术背景下的准确性。对于SK（假设为某种特定软件或框架）这样的专有名词，通常不会进行翻译，以保持在专业文档中的统一性和识别度。
...
1. **用户提交个人信息**：流程开始于用户向系统提供其个人详细信息以填写新的客户表格。
2. **填写新客户表单**：
   - 如果需要更多信息，系统会通过助理消息请求用户提供更多资料。
   - 填写完成后，系统发送欢迎信息给用户，并标记表单为已完成。
3. **完成表单后进行后续检查**：
   - **信用评分检查**：评估客户的信用分数。如果失败（信用分数不足），则通过邮件服务通知用户；若成功，则进入欺诈检测环节。
   - **欺诈检测**：如果身份验证失败，也会通过邮件服务通知用户；如果通过，则将新客户表单及对话记录传递给核心系统。
4. **核心系统记录创建**：在核心系统中创建记录，同时触发营销数据库、CRM记录创建以及生成欢迎包（包括账户详情）。
5. **营销与CRM记录创建**：一旦营销和CRM记录创建成功，即准备好了欢迎包。
6. **通知用户账户创建成功**：最后，通过邮件服务通知用户账户创建成功，标志着此次交互的结束。



**案例2-b**

- 在对具有共同主题/依赖关系的步骤进行分组，并创建较小的子流程并将其用作步骤后，根流程如下所示：
...
- 其中，用作步骤的流程（重复使用Step02a_AccountOpening使用的相同步骤）是：
...
本教程包含一组与餐厅的 Food Preparation Processes 相关的食物食谱。
...
```mermaid
flowchart LR
    PreparePotatoFriesEvent([准备土豆<br/>条事件])
    PotatoFriesReadyEvent([土豆条<br/>准备好事件])

    GatherIngredientsStep[收集原料<br/>步骤]
    CutStep[切食物<br/>步骤]
    FryStep[炸食物<br/>步骤]

    PreparePotatoFriesEvent --> GatherIngredientsStep -->| 切土豆<br/>_原料收集完成_| CutStep --> |**土豆已切好**<br/> _食物已切片就绪_| FryStep --> |_油炸食物就绪_|PotatoFriesReadyEvent
    FryStep -->|炸土豆失败<br/>_油炸食物失败_| GatherIngredientsStep
```
...
```mermaid
flowchart LR
    PrepareFriedFishEvent([准备炸<br/>鱼事件])
    FriedFishReadyEvent([炸鱼<br/>准备好事件])

    GatherIngredientsStep[收集原料<br/>步骤]
    CutStep[切食物<br/>步骤]
    FryStep[炸食物<br/>步骤]

    PrepareFriedFishEvent --> GatherIngredientsStep -->| 切鱼<br/>_原料收集完成_| CutStep --> |**鱼已切好**<br/> _食物已切块就绪_| FryStep --> |_油炸食物就绪_| FriedFishReadyEvent
    FryStep -->|**炸鱼失败**<br/> _油炸食物失败_| GatherIngredientsStep
```
...
```mermaid
flowchart LR
    PrepareFishSandwichEvent([准备鱼<br/>汉堡事件])
    FishSandwichReadyEvent([鱼汉堡<br/>准备好事件])

    FriedFishStep[[炸鱼<br/>过程步骤]]
    AddBunsStep[添加面包<br/>步骤]
    AddSpecialSauceStep[添加特殊<br/>酱料步骤]

    PrepareFishSandwichEvent -->|准备炸鱼| FriedFishStep -->|炸鱼就绪| AddBunsStep --> |面包已加| AddSpecialSauceStep --> |特殊酱料已加| FishSandwichReadyEvent
```
...
```mermaid
flowchart LR
    PrepareFishAndChipsEvent([准备<br/>鱼和薯条<br/>事件])
    FishAndChipsReadyEvent([鱼和薯条<br/>准备好事件])

    FriedFishStep[[炸鱼<br/>过程步骤]]
    PotatoFriesStep[[土豆条<br/>过程步骤]]
    AddCondiments[添加调味品<br/>步骤 ]

    PrepareFishAndChipsEvent -->|准备炸鱼| FriedFishStep --> |炸鱼就绪| AddCondiments
    PrepareFishAndChipsEvent -->|准备土豆条| PotatoFriesStep -->|土豆条就绪| AddCondiments
    AddCondiments -->|调味品已加| FishAndChipsReadyEvent
```

##### 有状态的产品准备过程

此部分中的过程包含了对先前使用的食物准备过程的以下修改/增加：

- `收集原料步骤`现在是有状态的，有一个预定义的初始原料数量，随着订单的准备而使用。当没有剩余原料时，它会发出`缺货事件`。
- `切食物步骤`现在是一个有状态组件，具有跟踪刀具锋利度的`刀具锋利度状态`。
- 每次调用`切食物`和`剁食物`功能时，刀具的锋利度都会恶化。
- `切食物步骤`还有一个额外的输入函数`磨刀功能`。
- 新的`磨刀功能`能磨锐刀具并增加刀具的锋利度 - 刀具锋利度状态。
- 不时地，`切食物步骤`的功能`切食物`和`剁食物`会失败，并发出一个`需要磨刀事件`，然后触发`磨刀功能`。

继续翻译您提供的笔记内容：

###### 带有刀具磨砺和原料库存管理的土豆条准备过程

以下是基于土豆条准备流程的过程修改，加入了之前提到的有状态步骤。

```mermaid
flowchart LR
    PreparePotatoFriesEvent([准备土豆<br/>条事件])
    PotatoFriesReadyEvent([土豆条<br/>准备好事件])
    OutOfStock([原料<br/>缺货<br/>事件])

    FryStep[炸食物<br/>步骤]

    subgraph GatherIngredientsStep[收集原料步骤]
        GatherIngredientsFunction[收集土豆<br/>功能]
        IngredientsState[(原料<br/>库存<br/>状态)]
    end
    subgraph CutStep ["切食物步骤"]
        direction LR
        SliceFoodFunction[切食物<br/>功能]
        SharpenKnifeFunction[磨刀<br/>功能]
        CutState[(刀具<br/>锋利度<br/>状态)]
    end
    
    CutStep --> |**土豆已切好**<br/> _食物已切片就绪_| FryStep --> |_油炸食物就绪_|PotatoFriesReadyEvent
    FryStep -->|炸土豆失败<br/>_油炸食物失败_| GatherIngredientsStep
    GatherIngredientsStep --> OutOfStock
    
    SliceFoodFunction --> |需要磨刀| SharpenKnifeFunction
    SharpenKnifeFunction --> |刀具已磨锐| SliceFoodFunction

    GatherIngredientsStep -->| 切土豆<br/>_原料收集完成_| CutStep
    PreparePotatoFriesEvent --> GatherIngredientsStep 
```

###### 带有刀具磨砺和原料库存管理的炸鱼准备过程

以下是基于炸鱼准备流程的过程修改，加入了之前提到的有状态步骤。
...
flowchart LR
    ...
    subgraph GatherIngredientsStep[收集原料步骤]
        GatherIngredientsFunction[收集鱼<br/>功能]
        IngredientsState[(原料<br/>库存<br/>状态)]
    end
    subgraph CutStep ["切食物步骤"]
        direction LR
        ChopFoodFunction[剁食物<br/>功能]
        SharpenKnifeFunction[磨刀<br/>功能]
        CutState[(刀具<br/>锋利度<br/>状态)]
    end
    
    CutStep --> |**鱼已剁好**<br/> _食物已切块就绪_| FryStep --> |_油炸食物就绪_|FriedFishReadyEvent
    FryStep -->|**炸鱼失败**<br/> _油炸食物失败_| GatherIngredientsStep
    GatherIngredientsStep --> OutOfStock
    
    ChopFoodFunction --> |需要磨刀| SharpenKnifeFunction
    SharpenKnifeFunction --> |刀具已磨锐| ChopFoodFunction

    GatherIngredientsStep -->| 剁鱼<br/>_原料收集完成_| CutStep
    PrepareFriedFishEvent --> GatherIngredientsStep 
```

**案例3-b**

- 目前，利用现有的产品准备流程，我们可以创建一个更为复杂的流程，该流程能够决定要配送哪些产品订单。

```mermaid
graph TD
    PrepareSingleOrderEvent([准备单个订单 <br/> 事件])
    SingleOrderReadyEvent([单个订单 <br/> 准备就绪事件])
    OrderPackedEvent([订单打包完成 <br/> 事件])

    DispatchOrderStep{{配送 <br/> 订单步骤}}
    FriedFishStep[[炸鱼  <br/> 处理步骤]]
    PotatoFriesStep[[薯条 <br/> 处理步骤]]
    FishSandwichStep[[鱼三明治 <br/> 处理步骤]]
    FishAndChipsStep[[炸鱼薯条 <br/> 处理步骤]]

    PackFoodStep[打包食物 <br/> 步骤]

    PrepareSingleOrderEvent -->|订单已接收| DispatchOrderStep
    DispatchOrderStep -->|准备炸鱼| FriedFishStep -->|炸鱼准备就绪| SingleOrderReadyEvent
    DispatchOrderStep -->|准备薯条| PotatoFriesStep -->|薯条准备就绪| SingleOrderReadyEvent
    DispatchOrderStep -->|准备鱼三明治| FishSandwichStep -->|鱼三明治准备就绪| SingleOrderReadyEvent
    DispatchOrderStep -->|准备炸鱼薯条| FishAndChipsStep -->|炸鱼薯条准备就绪| SingleOrderReadyEvent
...
- 本教程演示了将_Agent Framework_与进程集成，这包括直接的_agent_交互以及使用_AgentGroupChat_。流程概括：
...
- 涉及重点
  ...
  - 每次Manager响应用户后对history进行意图判断，涉及对大模型的json_schema响应格式化，反序列化为Indent对象决定下一步发出事件名称。
  - 每次启用agentGroup，先对之前用户和Manager的chatHistory总结，作为其输入。
